...
p1 <- ggplot() + 
  ...
  labs(title = "Normal Style")
p2 <- ggplot() + 
  geom_point(aes(x=mpg, y=wt), data=mtcars) + 
  labs(title = "XKCD Style") +
...
```{r horz_bar, echo=FALSE}
resumen <-
  structure(list(tonombre = structure(c(1L, 2L, 3L, 11L, 4L, 5L, 
                   8L, 6L, 7L, 9L, 10L, 14L, 12L, 13L, 15L), 
              .Label = c("Andalucía", "Aragón", "Asturias", "Canarias", "Cantabria", "C-LaMancha",
                          "CyLeón", "Cataluña", "Extremadura", "Galicia", "Baleares", "Madrid", "Murcia", 
                          "La Rioja", "Valencia"), class = "factor"), 
              persons = c(2743706L, 515772L, 364410L, 399963L, 699410L, 
                          212737L, 2847377L, 717874L, 894946L, 371502L, 942277L, 119341L, 
                          2561918L, 493833L, 1661613L), 
              frompersons = c(14266L, 3910L, 3214L, 3283L, 4371L, 1593L, 10912L, 
                              8931L, 9566L, 3231L, 5407L, 940L, 21289L, 3202L, 9939L), 
              topersons = c(10341L, 3805L, 2523L, 4039L, 3911L, 1524L, 12826L, 10897L, 
                            7108L, 2312L, 4522L, 1066L, 26464L, 3529L, 9187L), 
              llegan = c(0.38, 0.74, 0.69, 1.01, 0.56, 0.72, 0.45, 1.52, 0.79, 0.62, 
                          0.48, 0.89, 1.03, 0.71, 0.55), 
              sevan = c(0.52, 0.76, 0.88, 0.82, 0.62, 0.75, 0.38, 1.24, 1.07, 0.87, 0.57, 
                          0.79, 0.83, 0.65, 0.6)), 
              .Names = c("tonombre", "persons", "frompersons", "topersons",
                           "llegan", "sevan"), 
              row.names = c(NA, -15L), 
              class = "data.frame"
            )
...
lineaespana1 <-   xkcdline(mapping=aes(x=1-0.5,y=lleganespana,xend=15+0.5, yend=lleganespana, yjitteramount=0.051), data= resumenlargo, linetype=2,mask=FALSE)
lineaespana2 <- xkcdline(mapping=aes(x=1-0.5,y=-lleganespana,xend=15+0.5, yend=-lleganespana), yjitteramount=0.051, data= resumenlargo,linetype=2,mask=FALSE)
 
 
kk3 <- kk2  +   xkcdline(mapping=aes(x=as.numeric(tonombre)-0.5,y=-1.24,xend=as.numeric(tonombre)-0.5, yend=1.52, xjitteramount=0.151), data= resumenlargo, size=3,color="white") + lineaespana1 + lineaespana2
...
kk7 <- kk6 + annotate("text", x=9.3, y = 1.3, label="Unos vienen, otros se van",family="xkcd" ) +
  xkcdline(aes(x=xbegin,xend=xend,yend=yend,y=ybegin), yjitteramount=0.135,data=data.frame(xbegin=9.0, xend=7.2, ybegin=1.2, yend=1.3))
kk7
```

##### Example of Atlernative Bar Graph
```{r alt_bar, echo=FALSE}
